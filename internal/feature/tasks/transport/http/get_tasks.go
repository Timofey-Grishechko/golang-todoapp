...
func (h *TasksHTTPHandler) GetTasks(rw http.ResponseWriter, r *http.Request) {
	...
	taskDomains, err := h.taskService.GetTasks(ctx, userID, limit, offset)
	if err != nil {
		responseHandler.ErrorResponse(
			err,
			"failed to get tasks",
	...
	response := GetTasksResponse(taskDTOsFromDoamins(taskDomains))
...
func getUserIDLimitOffsetParams(r *http.Request) (*int, *int, *int, error) {
	const (
		userIDQueryParamKet = "user_id"
		limitQueryParamKey  = "limit"
		offsetQueryParamKey = "offset"
	)

	userID, err := core_http_request.GetIntQueryParam(r, userIDQueryParamKet)
	if err != nil {
